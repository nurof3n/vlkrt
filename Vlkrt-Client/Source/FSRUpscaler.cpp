...
namespace Vlkrt
{
    static FfxApiResource CreateFfxResource(std::shared_ptr<Walnut::Image> img, const wchar_t* name, uint32_t state)
    {
        ...
        switch (img->GetFormat()) {
            ...
            default: info.format = VK_FORMAT_R8G8B8A8_UNORM; break;
        }
    ...
    FSRUpscaler::~FSRUpscaler() { Shutdown(); }
    ...
    void FSRUpscaler::GetRenderResolution(
    ...
    void FSRUpscaler::OnResize(uint32_t displayW, uint32_t displayH, Quality q, float sharpness)
    {
        ...
        if (m_DisplayW == displayW && m_DisplayH == displayH && m_Quality == q && m_Sharpness == sharpness) return;
        ...
        if (err != FFX_API_RETURN_OK) {
            ...
            m_Status = "Ready (" + std::to_string(m_RenderW) + "x" + std::to_string(m_RenderH) + " -> "
                       + std::to_string(m_DisplayW) + "x" + std::to_string(m_DisplayH) + ")";
            WL_INFO_TAG("FSRUpscaler", "Created FSR upscaler context: {}x{} -> {}x{}", m_RenderW, m_RenderH, m_DisplayW,
                    m_DisplayH);
        }
    }

    void FSRUpscaler::Dispatch(VkCommandBuffer cmd, std::shared_ptr<Walnut::Image> colorIn,
            std::shared_ptr<Walnut::Image> depthIn, std::shared_ptr<Walnut::Image> motionIn,
            std::shared_ptr<Walnut::Image> colorOut, glm::vec2 jitter, float deltaTimeMs, bool resetHistory,
            float cameraNear, float cameraFar, float verticalFovRadians)
    {
        ...
        desc.motionVectors
                = CreateFfxResource(motionIn, L"FSR3_InputMotionVectors", FFX_API_RESOURCE_STATE_COMPUTE_READ);
        ...
        if (err != FFX_API_RETURN_OK) { WL_ERROR_TAG("FSRUpscaler", "ffxDispatch failed, error: {}", (int) err); }
    }
}  // namespace Vlkrt
